...
export default async function ComplaintsListingPage({
  searchParams
}: {
  ...
  // Get blotter data with filters (only for complaint type)
  const complaintData = await getBlotterData({
    status,
    priority,
    incidentType,
    searchTerm,
      ...
      <div className="p-6 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-gray-500 mt-1">
                View and manage all complaint entries
              </p>
            </div>
          </div>

          <Link href="/dashboard/blotter/new-complaint">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <Card className="bg-purple-50 border-purple-100">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-purple-700">Total Complaints</p>
                <p className="text-2xl font-bold text-purple-900">{complaintData.totalCount}</p>
              </div>
          ...
          <Card className="bg-yellow-50 border-yellow-100">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-900">{complaintData.pendingCount}</p>
              </div>
          ...
          <Card className="bg-green-50 border-green-100">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-900">{complaintData.resolvedCount}</p>
              </div>
          ...
          <Card className="bg-red-50 border-red-100">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-900">{complaintData.escalatedCount}</p>
              </div>
        ...
        <div className="mb-6 space-y-4">
          <div className="flex flex-col md:flex-row gap-4">
            <form className="relative flex-1" action="/dashboard/blotter/complaints" method="get">
            ...
            <div className="hidden md:flex flex-wrap gap-2">
              <form action="/dashboard/blotter/complaints" method="get" className="flex gap-2">
        ...
        <Card className="mb-6">
          <CardHeader className="pb-0">
            <CardTitle>Complaint Entries</CardTitle>
            <CardDescription>
              {complaintData.totalCount} {complaintData.totalCount === 1 ? 'entry' : 'entries'} found
              {searchTerm ? ` matching "${searchTerm}"` : ''}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {complaintData.cases.length > 0 ? (
              ...
              <div className="text-center py-12">
                <p className="text-gray-500">No complaint entries found matching your criteria.</p>
              </div>
            )}
            ...
            {complaintData.totalPages > 1 && (
              <div className="flex items-center justify-between mt-6">
                <div className="text-sm text-gray-500">
                  Showing <span className="font-medium">{(complaintData.currentPage - 1) * 10 + 1}</span> to{" "}
                  <span className="font-medium">
                    {Math.min(complaintData.currentPage * 10, complaintData.totalCount)}
                  </span>{" "}
                  of <span className="font-medium">{complaintData.totalCount}</span> results
                </div>
                <div className="flex space-x-2">
                  <Link
                    href={`/dashboard/blotter/complaints?${new URLSearchParams({
                      ...(status ? { status } : {}),
                      ...(priority ? { priority } : {}),
                      ...(incidentType ? { incidentType } : {}),
                      ...(searchTerm ? { q: searchTerm } : {}),
                      page: (complaintData.currentPage > 1 ? complaintData.currentPage - 1 : 1).toString()
                    }).toString()}`}
                    className={`inline-flex items-center px-3 py-1 rounded border ${complaintData.currentPage === 1
                      ? "border-gray-200 text-gray-300 cursor-not-allowed"
                      : "border-gray-300 text-gray-600 hover:bg-gray-50"
                      }`}
                    aria-disabled={complaintData.currentPage === 1}
                    tabIndex={complaintData.currentPage === 1 ? -1 : 0}
                  >
                  ...
                  <Link
                    href={`/dashboard/blotter/complaints?${new URLSearchParams({
                      ...
                      ...(incidentType ? { incidentType } : {}),
                      ...(searchTerm ? { q: searchTerm } : {}),
                      page: (complaintData.currentPage < complaintData.totalPages ? complaintData.currentPage + 1 : complaintData.totalPages).toString()
                    }).toString()}`}
                    className={`inline-flex items-center px-3 py-1 rounded border ${complaintData.currentPage === complaintData.totalPages
                      ? "border-gray-200 text-gray-300 cursor-not-allowed"
                      : "border-gray-300 text-gray-600 hover:bg-gray-50"
                      }`}
                    aria-disabled={complaintData.currentPage === complaintData.totalPages}
                    tabIndex={complaintData.currentPage === complaintData.totalPages ? -1 : 0}
                  >
                    Next
